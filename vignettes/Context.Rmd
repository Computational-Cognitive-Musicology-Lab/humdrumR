...
```{r}
chorales |> 
  mutate(Beat = timecount(Token, beat = tactus(TimeSignature))) -> chorales
...
Another common example would be `tally()`:
...
For functions that accept unlimited arguments (`...`), like `paste()`, and `tally()`, you can easily extend the principle to create longer n-grams:
