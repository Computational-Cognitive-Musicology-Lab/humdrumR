...
This article explains how `r hm` can be used to contextualize musical data.
...
By default, the `mutate()` function will [recycle](../reference/recycling.html) any scalar (length one) result throughout its group,
...
We use `context()` by indicating when to "open" (begin) and "close" (end) contextual windows, using the `open` and `close` arguments.
...
There are many ways to control how contextual windows are defined, and it's often difficult to decide what we want in a particular analysis.
...
The `context()` function can handle nested windows by setting `overlap = 'nested'`.
...
Depending on what you are doing with you n-grams, `context()` may be the only way that works---basically, if you want to apply an expression separately to each n-gram, you need to use `context()`.
...
The [with/dplyr](../reference/withinHumdrum.html) functions allow you to create lagged vectors in a special, concise way.
...
We can then paste a field (like `SciPitch`) to itself *lagged*, like this:
...
We can use the R function `grep(pattern = 'NA', invert = TRUE, value = TRUE)` to get rid of these:
...
Actually, it is right: in these 10 chorales, there is only one 10-gram pitch pattern that occurs more than once!
