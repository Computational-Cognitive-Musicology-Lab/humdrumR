...
Using `within.humdrumR()`'s `groupby` argument, we can perform calculations grouped by record.
...
Most records have new notes in all four voices---Not surprising for choral music---with *one* onset being the next most common case.
Note that we include `File` in our groupby argument (e.g., `by = list(File, Record)`).
Why? Because each piece/file in the data set repeats the same record numbers---you wouldn't want to count the 17th record from the 3rd chorale together with the 17th record from the 7th chorale, for example.
...
The highest lowest-note-in-bar is `A`.

---

For further analysis, we might want to save the lowest-note into a new field.
Let's use `within.humdrumR`'s `fill = ` option to duplicate the low note throughout the bar:
...
There will be no automatic "beat" field in our data, we'll need to make it---obviously, we need ti have data with rhythmic information encoded (like `**kern` or `**recip`).
...
We can now group by beat.
Maybe we want to know the range of notes in each beat:

```{r}

with(chorales, diff(range(Semits)), by = list(File, Beat)) |> hist()


```


> Note: If your data includes spine paths, you'll want to set `within(mydata, count(Token), expandPaths = TRUE, ...)`.
> Count (and similar functions, like `timeline()`) won't work correctly without paths expanded (`?expandPaths`).

# Contextual Windows

When you use `groupby`/`by`, your data is exhaustively partitioned into non-overlapping groups; the groups are also not necessarily ordered---we have to explicitely used (temporally) ordered groups like `list(File, Record)` if we want temporal context.
In contrast, the `context()` function, when used in combination with `with.humdrumR()`, gives us much more flexible control over the context we want for our data.
...
The way you use `context()` is by telling it when to "open" (begin) and "close" (end) contextual windows, using the `open` and `close` arguments.
...
For most examples, we'll use `within.humdrumR()` and run the command `paste(Token, collapse = '-')`.
...
Cool, we've got overlapping four-note windows, running throught each spine!
How did we do this?
...
When we use `hop()` inside `context()`, it automatically knows what the input vector is.
...
We don't have to define `open` first, then have `close` refer to open---we can also do the opposite!
...
```{r}
within(chorales$Token, solfa(Token, simple = TRUE) -> Solfa) -> chorales
```
...
```{r, eval = FALSE}
with(chorales$Solfa, paste(Solfa, collapse = '-'), context('ti', 'do'))
```
```{r, echo = FALSE}
with(chorales$Solfa, paste(Solfa, collapse = '-'), context('ti', 'do')) |> unname()
...
```{r, eval=FALSE}
with(chorales$Solfa, paste(Solfa, collapse = '-'), context('ti', 'do', overlap = 'none'))
```
```{r, echo = FALSE}
with(chorales$Solfa, paste(Solfa, collapse = '-'), context('ti', 'do', overlap = 'none')) |> unname()
...
within(chorales$Token, duration(Token) -> Duration) -> chorales
...
```{r, eval = FALSE}
with(chorales$Solfa, paste(Token, collapse = '-'), context('ti', Solfa == 'do' & Duration >= .5, overlap = 'edge'))
```
```{r, echo = FALSE}
with(chorales$Solfa, paste(Token, collapse = '-'), context('ti', Solfa == 'do' & Duration >= .5, overlap = 'edge')) |> unname()
```

Notice that, because our `close` expression is more complicated now, I had to explictely say `Solfa == 'do'` instead of using the shortcut of just providing a single string.
...
```{r, eval = FALSE}
with(chorales$Token, paste(Token, collapse = '-'), context(prevclose + 1, ';'))
```
```{r, echo = FALSE}
with(chorales$Token, paste(Token, collapse = '-'), context(prevclose + 1, ';')) |> unname()
```
...
You can fix this by explicitely adding an opening window at `1`:

```{r, eval = FALSE}
with(chorales$Token, paste(Token, collapse = '-'), context(1 | prevclose + 1, ';'))
```
```{r, echo = FALSE}
with(chorales$Token, paste(Token, collapse = '-'), context(1 | prevclose + 1, ';')) |> unname()
```

By using the `|` (or) command in `context()`, we are saying open a window at `1` *or* at `prevclose + 1`.
When working with `nextopen` you might want to use the special `end` argument (only available inside `context()`), which is the last index in the input vector.0
...
To see some examples, let's load our built-in Beethoven variation dataset and remove multi-stops (which would make this much more complicated).
...
Most of the slurts are only 2, 3, or 4 notes.
...
Ok, what if we want to collapse (`paste()`) our slurred notes together, like we've been doing throughout this article?
...
We can keep those tokens by adding a `complement` argument to `within.humdrumR()`---this works the same was as the complement
when you do a subset inside `within.humdrumR()` (see the [Working with humdrum data](WorkingWithData.html) article).
Basically, any indices in the active field that *aren't* captured in any of your windows, are in the complement.
If we want to keep the original tokens there, just pass `complement = Token`.
