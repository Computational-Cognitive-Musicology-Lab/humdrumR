...
axis(2, ax, as.tonalname(as.character(ax)), tick = FALSE, las = 1)
...
In pythagorean tuning, enharmonic notes---tonal chroma separated by 12 steps on the line-of-fifths, like G# and Ab---are not exactly the same.
However, when tuning many musical instruments, musicians are forced to "pick a side"---is it G# or Ab? 
(This corresponds to a modulo 12 operation on the line-of-fifths, creating the circle-of-fifths.)
...
method      `sigma`  `delta`  `contour`  `Key`  `generic`  `cautionary`  `simple`  `octave`  `direction`  
----------- -------- -------- ---------- ------ ---------- ------------- --------- --------- ------------
`as.semit`  
