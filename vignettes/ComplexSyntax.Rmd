...
`r Hm`'s [tidyverse methods](../reference/withinHumdrum.html), like `mutate()` and `within()`, also have an `expandPaths` argument.
...
In `r hm`, we call these "Stops"---as always, **every** humdrum token, including stops, get their own row in a `r hm` [humdrum table](../reference/humTable.html).
...
In other analyses---especially, if you are analyzing data in a linear/melodic way---, incorporating/handling multiple stops may be very difficult, with no obviously correct way to do it.
