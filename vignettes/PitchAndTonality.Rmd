...
We can use the `Key` argument to control how our pitch functions interpet key---either translating *from* degrees to absolute pitches, or vice versa.
...
with(chorales, semits(Token)) |> hist(main = 'Histogram of semits() in 10 chorales')
...
If we use a pitch function in `with()` or `within()`, they will *automatically* pass the `Key` field as `Key` argument to the function!
...
The rest token, is output as a null (`NA`) value.
...
If `generic == TRUE`, the only generic pitch information is returned.
...
The `specific` argument is simply the opposite of `generic`, so have the choice of indicating `genric = TRUE` or `specific = FALSE`---they are equivalent.
...
+ For `interval()`, only steps between 1--7 and reterned (i.e., no 10ths).


The `compound` argument is simply the opposite of `simple`, so have the choice of indicating `simple = TRUE` or `compound = FALSE`---they are equivalent.
...
The `generic` and `simple` arguments useful if you want to tabulate pitch classes, for example:
...
with(chorales, kern(Token, generic = TRUE)) |> table() |> barplot()

```

```{r}

with(chorales, kern(Token, simple = TRUE, generic = TRUE)) |> table() |> barplot()
...
You can use this functionality by passing arguments to a `transpose()` call, *inside* the pitch function call, like in this example using the `by` argument:

```{r}

kern(c('c', 'd', 'e', 'f', 'f#', 'g'), transpose(by = 'M2'))

```

Another option is to transpose by key.
You can pass the `transpose()` command a `to` argument.
...
So the `Key` argument to `kern()` indicates the key the music is coming from, and the `to` argument in `tranpose()` indicates where to transpose it to.
Since `with`/`within` calls automatically use `Key` information from a data set, this makes it very easy to, say, transpose all `**kern` tokens in your data to C major:
...
with(chorales, solfa(Token, simple=T)) |> table() |> barplot()
```
...
If you use `mint()` in `with`/`within` call on a `r hm` dataobject, `mint()` will automatically be called within each spine/path in each file (this is accomplished using the `groupby` argument to `mint()`):
...
For example, you can caculate harmonic intervals between each voice and the bass voice like so:
