...
`r Hm`'s pitch functions can all be used in [humdrum-style](DataFields.html#humdrum-style "HumdrumR data fields article, humdrum-style section"), so using `semits()` is very easy:
...
The complete list of these pitch arguments can be found in the `?pitchParsing` and `?pitchDeparsing` pages, but the most important ones are described here:
