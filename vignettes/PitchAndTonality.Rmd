...
For the most part, `humdrumR`'s pitch tools are focused on the Western system of tonality, and tools for repretesenting pitches in a tonal context are the focus of *this* vignette---we also have standard tools for looking at pitch from a Western, 12-tone atonal setting as well.
...
Fortunately, many sections are marke with "tl;dr" summaries, so you can skip to (sub)sections that are relavent to you.
...
We percieve the distance (interval) between pitches based on the ratio between their frequencies.
...
Pitches separated by an octave are *so* consonant that we actually percieve them as being "the same pitch" (in some respects).
...
Thus, even though we can percieve close to ten octaves worth of frequencies, the organization of pitch in music is mostly based on pitches that can be squeezed into a single octave.
...
> Most tonal analyses don't really depend on the specific flavor of Western tuning/temperement you use.
