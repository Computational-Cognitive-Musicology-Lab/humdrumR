...
An alternative way of using the `start` argument is be feeding the `start` argument a `logical` vector the same length as the input (`x`): the first `TRUE` value in the `start` vector is chosen as the start of th timeline.
This is useful for specifying a pick-up.
...
Since we can see that the pickup has a duration of `.25` (quarte-note), we *could* do this:
...
However, we could also use the `logical` approach---we want the downbeat (`0`) to be on that `2e`, so

```{r}
timeline(melody, start = melody == '2e')

```

---

A common technique for identifying pickups in `humdrumR` datasets is be looking at the `Bar` field.
The first downbeat will typically be the first place the `Bar == 1`---pickups will have `Bar == 0`.
So, in our Bach chorales:

```{r}

within(chorales, timeline(Token, start = Bar == 1))
