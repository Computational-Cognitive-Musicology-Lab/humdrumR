...
In many cases, we actually want to know what timeline position for *every* token in a file is.
This is especially useful when your data set contains some data spines which don't explicitely have rhythmic information
but which line up with the rhythmic information in other spines (as they generally should!).
A good example if our rap flow (MCFlow data), which contains rhythm encoded in one `**recip` spine, lined up with
lots of other information in other spines.
If we load our MCFlow data and run `timeline()`, we'll see the timeline computed for the `**recip` spine:
...
That's all well and good, but what if you want to know at what point in the timeline the Fresh Prince first mentioned "summertime"?
It would be helpful to get the "total" timeline for the file, by specifying `total = TRUE`:
...
Thus, the *first* `FALSE` value in the `pickup` vector is chosen as the start of the timeline.
...
The first thing we might want to do, is to take a sequence of rhythm durations and identify which metric level each onset lands on.
...
Along with `metlev()`, `r hm` provides two complementary functions called `metcount()`, `metsubpos()`.
