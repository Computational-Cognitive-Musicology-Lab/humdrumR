...
This article explains how you can find essential metainformation about your data with `r hm`: how much data is there, and how is it structured?
...
The corpus contains, in total, `r humdrumR:::num2print(sum(census(chorales)$Tokens))` tokens in `r humdrumR:::num2print(sum(census(chorales)$Records))` records.
...
The next thing to do, when getting started with a `r hm` data analysis, is to get a sense of the data *content* itself.
...
But what is the `X` in `4dnX`? Or all the `J`s and `L`s and `;`s?
