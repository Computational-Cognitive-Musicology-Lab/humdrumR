...
This is a `r hm` workhorse function, which is used to count the number of unique values in a vector, or unique combinations of values across combinations of vectors.
...
So in `r hm`, `count()` will (attempt) to treat numeric values in a smart way, by binning them into ranges using the base R `hist()` function's binning algorithm.
So for example:
...
Deatils of this process can be controlled using the `binArgs` argument, which must be a list of arguments.
In particular, you pass arguments to the `hist()` alrgorithm, such as `breaks` (to control the number of bins).
