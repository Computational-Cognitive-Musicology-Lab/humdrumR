...
The humdrum data you've imported is stored in a `r hm` data object, which gives us some special tools for working with it.

Let's load our prepackages `chorales` dataset, as we did in our [quick start](GettingStarted.html#quick-start "Getting started with humdrumR article") "Reading and writing humdrum data").
...
The key functions for working with `r hm` data objects are `with` and `within`.
...
You should call `with` with a `r hm` data object as the first argument.
...
In many cases, we want to apply functions to our humdrum data, but keep the humdrum/`r hm` structure in place.
...
"back inside" the `r hm` data object, as a new field.
...
Now we get our `r hm` data back, but the `Token` data has been transformed from `**kern` to `**pitch`.
...
Two prominent examples allow you to automatically work with subsets of your data: the `subset` and `by` arguments.
We can indicate a subset, or multiple subsets, and our expressions will 
be evaluated separately within each subset.

## With Subset

The `subset` argument indicates a subset of the data which we should evaluate "within."
There `subset` argument must be an expression which evaluates to a logical vector (`TRUE` of `FALSE`).
The main command is then evaluated wherever the `subset` expression evaluates to `TRUE`.
...
If we want even numbers, we can ask where is `Bar` modulo 2 zero?: `Bar %% 2 == 0`.
...
If we use the `subset` argument with `within`, parts of the data which don't match the `within` expression
...
In many cases when using `subset` we'd like to do something with, or at least keep, the "other" part of the data---the complement of the subset.
We can do this by expressing a "complement" expression.
The idea is that we say "evaluate X where Y is true, otherwise evaluate this instead."
This is easiest to understand if we use our `within` example from above.
...
by using the `recycle` argument for our expression.
