...
If you want to be more precise about filtering, use the `subset` method for `humdrumR` data.
`subset.humdrumR` works exactly like [within.humdrumR](WorkingWithData.html "Working with Humdrum Data").
However, the expression(s) you give must evaluate to a logical vector (`TRUE` or `FALSE`) of the same length as its input.
...
We can use the `any` function to get *any* match, and the `fill` option to copy the result of `any` to the whole bar.

```{r}

subset(chorales, fill = any(Token %~% '-'), by = list(File, Bar))
...
subset(chorales, fill = any(Token %~% '-'), by = list(File, floor(Bar  / 2)))
...
By using this commands, we make sure that 1) you explicitly want to remove them and 2) the humdrum syntax is not broken, because whole records/spines/paths/files are removed.
