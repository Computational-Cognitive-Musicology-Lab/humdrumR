...
However, when it comes to analyze data, we typically want to "reshape" our data into a particular format that is ideal for *analysis*.
...
The easiest way to do this is to index out spines we don't want, either using numeric indices
...
The classic example, of course, is `**kern` itself which often includes rhythm, pitch, phrasing, beaming, and pitch ornamentation information!
...
What if we actually want to move the rhythm and pitch information to separate spines---i.e., into their rows in the humdrum table?
Use `rend()`:
...
Currently, by default, each `**kern` token *and* each `**silbe` token are in their own, separate row of the humdrum table ("row" in terms of `view = "table"`, rather than spines in `view = "humdrum"`).
...
In these cases, it will be much smarter to pass `cleave()` the names of the exclusive interpretations we want to cleave.
...
Since we are cleaving by the name of exclusive interpretation here, `r hm` names the new field with the capitalized exclusive interpretation name by default.
...
As they are, this would be hard because the tokens from each spine are all in their own rows (in the table view).
...
By default, `r hm` treats each token (note) in *each* stop as a separate data observation, with its own row in the humdrum table view.
