...
The essential thing to understand is how it maps between the [humdrum data syntax](humdrum.org) and a R data.table.
...
This file contains `r num2word(length(tokens))` individual tokens.
...
Look at that, a `data.table` with `r num2word(nrow(humtab))` rows!
In this case, the humdrum data table ("humtable") has `r num2word(ncol(humtab))` columns!
The columns are:
...
We see that there are `r num2word(ncol(humtab))` fields (humtable columns), and that they are divided into five types of fields: 
...
For instance, lets use `humdrumR`'s `as.semit` function (see the [humdrum pitch](Pitch.html) vignette) to calculate the semitone value for each pitch in the chorales:
