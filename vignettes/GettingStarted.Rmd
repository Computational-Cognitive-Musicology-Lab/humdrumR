...
These simplest way is using the `wtihin` function, which takes a `humdrumR` object (on its left side), and an expression on its right side.
...
You'll also notice that in the printout, there is a new Data field, called `Result1`---this is the default name `humdrumR` assigns to new fields that you create.
