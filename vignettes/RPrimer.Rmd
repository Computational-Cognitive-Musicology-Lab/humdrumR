...
As you can see, you can do arithmetic in very intuitive ways, like `5 / 2` or `3 * 3` (more details lin a later section).
...
For example, something like `log(round(sqrt(mean(x^2)), base = 2)` calls *four* functions!
...
For example, we can re-write the previous command as:
...
For example, `X1` or `my_name` are valid names---but noat `2X`.
...
The one exception is that you can call `data.frame` with some *scalar* single values, which will be automatically recyced to match
