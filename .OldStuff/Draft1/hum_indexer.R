...
hum_grep = function(regexes, score, margin, return_logical = TRUE){
  ...
  if(missing(regexes) || length(regexes) == 0){
    if(return_logical) return(rep(TRUE, dim(score@Data)[other_margin])) else return(seq_len(dim(score@Data)[other_margin]))
  }
  ...
  ## --
  Data  = list(list(Data    = unlist(apply(score@Data   , 1, list), recursive = FALSE),
                    Tandems = unlist(apply(score@Tandems, 1, list), recursive = FALSE)),
               list(Data    = score@Data,
