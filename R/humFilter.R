...
#' Filter humdrum data
#' 
#' \code{filterHumdrum} is a command used to filter a \code{\linkS4class[humdrumR]{humdrumR}} corpus.
#' Indexing (using the \code{[]} or {\code{[[]]}} operators is a special case---indeed,
#' the \code{[]} and \code{[[]]} methods for \code{\linkS4class[humdrumR]{humdrumR}} objects are 
...
#' of rows in the \code{\link[humtable]{humdrum table}}) \emph{or} be of length \code{1}. 
#' 
#' If a predicate expression \emph{is} of length \code{1}, the entire \code{\link[humtable]{humdrum table}}
...
filterHumdrum <- function(humdrumR, ..., indexGLIM = TRUE) { 
    ...
    if (any(sapply(formulae, Negate(rlang::is_formula)))) stop('In filterHumdrum(...) unnamed arguments must be formulas or functions.')
...
# humdrumR filtering is usually applied in reference to data records.
...
# and strip away GLIMd records that are defunct (because data
# records have been stripped away).
# We also need to bad records and spines with null data tokes (sometimes).
