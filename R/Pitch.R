...
#' 
#' All pitch formats have the option to be output ([deparsed][pitchDeparsing]) as R [factors][factor()].
#' (See the `vignette('RPrimer')` for an explanation of what factors are.)
#' For `character` representations, the `as.factor` argument defaults to `TRUE`; for `numeric` 
#' representations, `as.factor` defaults to `FALSE`---note that if `inPlace = TRUE`,
#' `as.factor` is forced to `FALSE`.
#' 
#' As factors, the pitch data will implicitly remember the [gamut()] of possible pitches,
#' and their order, so when you call [table()], the resulting table is well formatted.
#' The gamut is generated by the [gamut()] function: the `simple`/`complex` and 
#' `generic`/`specific` arguments are automacitally passed to [gamut()]; additional
...
makePitchTransformer <- function(deparser, callname, 
                                 ...
                                 keyed = TRUE, factor.default = TRUE,
...
#' @export 
freq  <- makePitchTransformer(tint2freq, 'freq', 'numeric', factor.default =FALSE, 
...
#' @inheritSection pitchDeparsing Pitch-Gamut Factor Levels
#' @export 
semits <- makePitchTransformer(tint2semits, 'semits', 'integer', factor.default =FALSE)


#' @rdname semits
#' @export 
midi  <- makePitchTransformer(tint2midi, 'midi', 'integer', factor.default =FALSE)
...
#' @rdname semits
#' @export 
cents  <- makePitchTransformer(tint2cents, 'cents', 'numeric', factor.default =FALSE, 
                               extraArgs = alist(tonalHarmonic = 2^(19/12)))
