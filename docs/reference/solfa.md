...
If `solfa()` is applied to a [humdrumR data
...
results
#> **solfa (character)
#> [1] vvso vso  vmi  vfi  vso  vre  vmi 
...
results
#> **solfa (character)
#>  [1] .    vdo  so   vti  so   vdo  so   vvfa vfa  vvfa vfa  vso  vmi  vso  vre 
#> [16] vdo  .    mi   ^so  do   mi   so   do   do   mi   mi   ^so  mi   ^so  re  
#> [31] fa   do   mi   do   ^la  do   ^la  do   ^so  vfa  ti   vmi  do   .   
```
