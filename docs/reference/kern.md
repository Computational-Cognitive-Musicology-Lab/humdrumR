...
Kern (`**kern`) is the most common humdrum interpretation for
representing "notes" in the style of traditional Western scores.
However! In
[humdrumR](https://humdrumR.ccml.gtcmt.gatech.edu/reference/humdrumR.md),
the `kern` function outputs the *pitch* part of the `**kern`
interpretation. `**kern` *rhythms* are instead created using the
[`recip()`](https://humdrumR.ccml.gtcmt.gatech.edu/reference/recip.md)
function.

If `kern()` is applied to a [humdrumR data
...
The pitch part of `**kern` tokens breakdown tonal pitch information as
so:
...
- **Octave**
  ...
  - Octave is indicated through the case of the step characters, as well
    as *repetition* of the step character. Uppercase letters are used
    for octaves below ; lowercase letters for the middle-**C** octave
    and higher. The octave, and the octave below it get one character
    each, with higher and lower octaves repeating that character. For
    example, using `C#` as the step value, and relative to the octave:
...
Like all `humdrumR` pitch functions, the ways that `kern`
[parses](https://humdrumR.ccml.gtcmt.gatech.edu/reference/pitchParsing.md)
and
[deparses](https://humdrumR.ccml.gtcmt.gatech.edu/reference/pitchDeparsing.md)
tokens can be modified to accomodate variations of the standard `**kern`
...
and `kern()`) have an option to keep the "extra" information and return
the result "in place"—i.e., embedded right where it was found in the
input string. This is controlled with the `inPlace` argument, which is
`FALSE` by default. So, `pitch('4.ee-[', inPlace = TRUE)` will return
4.Eb5\[—keeping the `"4."` and the `"["`. (This obviously only works if
the input is a string, not a numeric!) Note that `inPlace = TRUE` will
force functions like `semits`, which normally return `numeric` values,
to return `character` strings *if* their input is a character string.
...
#>  REpath-pattern '/home/nat/.tmp/RtmpjOz08Z/temp_libpath13452e707787ad/humdrumR/HumdrumData/BeethovenVariations/B075_00_05_a.krn' matches 1 text files in 1 directory.
...
results
#> **kern (character)
#>  [1] .   F   c   E   c   F   c   BB- B-  BB- B-  C   A   C   G   F   .   a   cc 
#> [20] f   a   c   f   f   a   a   cc  a   cc  g   b-  f   a   f   dd  f   dd  f  
#> [39] cc  B-  e   A   f   .  
