...
This is the representation used to represent (Western tonal) pitches in
the
[Lilypond](https://lilypond.org/doc/v2.22/Documentation/notation/pitches)
notation format. In
[humdrumR](https://humdrumR.ccml.gtcmt.gatech.edu/reference/humdrumR.md),
the `lilypond` function only relates to the *pitch* part of Lilypond
notation: Lilypond-like *rhythms* can be creating using the
[recip](https://humdrumR.ccml.gtcmt.gatech.edu/reference/recip.md)
function.

If `lilypond()` is applied to a [humdrumR data
...
results
#> **lilypond (character)
#>  [1] .    f    c'   e    c'   f    c'   bes, bes  bes, bes  c    a    c    g   
#> [16] f    .    a'   c''  f'   a'   c'   f'   f'   a'   a'   c''  a'   c''  g'  
#> [31] bes' f'   a'   f'   d''  f'   d''  f'   c''  bes  e'   a    f'   .   
```
